...
class Sorting:
    def __init__(self, root, AlgoNameVar):
        ...
        self.frame1 = Frame(root, width=self.FRAME1_X, height=self.FRAME1_Y, bg="white")
        self.frame1.grid_propagate(0)
        self.frame1.pack(side=LEFT)


        # Algorithm Information Table
        self.information = {'Merge Sort': "Worst Case:O(n*log n)\nAverage Case:O(n*log n)\nBest Case:O(n*log n)",
                            'Insertion Sort': "Worst Case:O(n²)\nAverage Case:O(n²)\nBest Case:O(n)"}


        # Algorithm Names
        self.algorithm = ['Insertion Sort', 'Merge Sort']
        ...
        self.frame_radio = Frame(self.frame1, bg="white", width=230, height=25, relief="flat", bd=4)
        self.frame_radio.place(relx=0, rely=0.23)
        self.frame_radio.grid_propagate(0)
        # Creating the button / option
        self.bar_drawing = Radiobutton(self.frame_radio, text="Bar", bg="white", fg="navy", variable=self.graph_type, value=0,
                                       font=("Helvetica", 10, "bold"), command=self.draw_type, cursor="hand2")
        self.color_drawing = Radiobutton(self.frame_radio, text="Color", bg="white", fg="navy", variable=self.graph_type,
        ...
        self.label_comparison = Label(self.frame1, text=" No. of comparisons: 0", bg="white", fg="midnight blue", font=("Fixedsys", 12))
        ...
        self.label_avg = Label(self.frame_algo_info, bg="tomato", fg="white", text=self.information[self.algo_var.get()], font=("comic sans ms", 13, "bold"))
        self.label_avg.pack_propagate(0)
        self.label_avg.place(relx=0.06, rely=0.25)


        # Back button to the main window
        self.BackButton = Button(self.frame1, bg="blue", fg="white", text="< Go Back to main menu", command=self.Back, cursor="hand2")
